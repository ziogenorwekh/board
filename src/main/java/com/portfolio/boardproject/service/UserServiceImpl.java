...
public class UserServiceImpl implements UserService {
    ...
    public UserCreateResponse createUser(UserCreateCommand userCreateCommand) {
        ...
        return new UserCreateResponse(saved.getUsername(), saved.getEmail(), saved.getCreatedAt());
    }

    @Override
    public UserTrackQueryResponse findUserById(UserTrackQuery userTrackQuery) {

        UserEntity userEntity = userRepository.findById(userTrackQuery.getUserId()).orElseThrow(() -> new RuntimeException());
    ...
    public void updateUser(UserUpdateCommand userUpdateCommand) {
        UserEntity userEntity = userRepository.findById(userUpdateCommand.getUserId()).orElseThrow(() -> new RuntimeException());
        User user = new User(userEntity);
